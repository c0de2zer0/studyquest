...
export interface User {
  ...
  streak: number; totalHours: number;
  rank: string; rankEmoji: string; lp: number;
  rankTier: string; rankDivision: number; lpHistory: number[];
  bio: string; joinDate: string; followers: number; following: number; friends: number;
  isVerified: boolean; collectionCount: number; collectionTotal: number;
  status: 'online'|'studying'|'break'|'dnd'|'offline';
  avatar: { currentForm: string; tier: string; rarity: string; glowColor: string; };
  stats: { focus: number; speed: number; endurance: number; memory: number; };
  equippedItems: { hat: string|null; top: string|null; bottom: string|null; shoes: string|null; hair: string|null; accessory: string|null; background: string|null; };
  featuredBadges: string[]; featuredItems: string[];
  weeklyXp: number; weeklyXpPrev: number;
  todayHours: number; dailyGoal: number; examDaysLeft: number; examGoalHours: number;
  showRankUpModal?: boolean;
  rankUpInfo?: { newTier: string; newDivision: number } | null;
}

export interface Task {
  id: string; name: string; subject: string; subjectColor: string; subjectEmoji: string;
  startTime: string; endTime: string; duration: number; coins: number; status: 'done'|'active'|'pending'; xp: number; elapsed?: number;
}

export interface Friend {
  id: string; name: string; emoji: string; level: number;
  status: string; statusText: string; statusColor: string; online: boolean;
}

export interface LeaderboardEntry {
  rank: number; name: string; emoji: string; level: number; score: number; xp: number;
  maxXp: number; trend: number; totalHours: number; isMe?: boolean;
}

export interface MarketItem {
  id: string; name: string; category: string; emoji: string; rarity: string;
  price: number; owned: boolean; equipped: boolean;
  isDaily?: boolean; dailyPrice?: number; costumeSlots?: Record<string,string>;
}

export interface ChatMessage {
  id: string; userId: string; userName: string; userEmoji: string; userColor: string;
  content: string; timestamp: string; channel: string;
  reactions: { emoji: string; count: number }[]; isSystem?: boolean;
}

export interface Channel {
  id: string; name: string; category: 'lessons'|'general'; unread: number;
}

export interface VoiceRoom {
  id: string; name: string; emoji: string; capacity: number; occupied: number;
}

export interface OnlineMember {
  id: string; name: string; emoji: string; level: number;
  status: string; statusText: string; color: string;
}

export interface Tournament {
  id: string; emoji: string; name: string; rule: string;
  status: 'active'|'tomorrow'|'open'|'soon'|'upcoming';
  color: string; prize: string; participants: number; timeLeft: string;
  joined: boolean; myRank: number|null;
}
...
export interface Notification {
  id: string; type: string; text: string; time: string; read: boolean; emoji: string;
}

export interface Badge {
  id: string; emoji: string; name: string; description: string;
  unlocked: boolean; date?: string; condition?: string;
}

export interface SubjectDistribution {
  subject: string; pct: number; color: string;
}

export interface EvolutionTier {
  id: string; emoji: string; name: string; reqHours: number;
  unlocked: boolean; current: boolean;
}

export interface CommunityPost {
  id: string; type: PostType;
  userId: string; userName: string; userEmoji: string; userColor: string;
  timestamp: string; content: string;
  reactions: { emoji: string; count: number; reacted?: boolean }[];
  subject?: string; subjectColor?: string; durationMin?: number;
  participants?: string[];
  isVerifiedChannel?: boolean; channelName?: string; followerCount?: number;
  channelEmoji?: string; channelBorderColor?: string;
  ...
  battleSubject?: string; battleDurationMin?: number; timeRemainingSeconds?: number;
  tournamentName?: string; tournamentPrize?: string; tournamentCapacity?: number;
  tournamentParticipants?: number; tournamentStartTime?: string;
}
...
export interface RoomMessage {
  id: string; userId: string; userName: string; userEmoji: string;
  content: string; timestamp: string;
}

export interface Room {
  id: string; name: string; subject: string; subjectColor: string;
  emoji: string; ownerId: string; ownerName: string; capacity: number;
  members: RoomMember[]; permissions: { canWrite: boolean; canCompete: boolean };
  isOpen: boolean; messages: RoomMessage[];
}

export const mockUser: User = {
  id: 'u1', name: 'Shadow Fox', emoji: '🦊',
  level: 14, xp: 3420, xpMax: 5000, balance: 212.5,
  streak: 23, totalHours: 247,
  rank: 'Altın IV', rankEmoji: '🥇', lp: 45,
  rankTier: 'Altın', rankDivision: 4,
  lpHistory: [32, 44, 38, 52, 48, 55, 42, 37, 61, 45, 50, 48, 62, 40, 35, 58, 45, 52, 39, 45],
  bio: 'YKS hazırlık · Matematik ❤️ · Hedef: 600 sa',
  joinDate: 'Eylül 2024',
  followers: 34, following: 21, friends: 12,
  isVerified: false, collectionCount: 12, collectionTotal: 30,
  status: 'studying',
  avatar: { currentForm: '🦊', tier: 'Tilki Evrimi', rarity: 'Nadir', glowColor: '#7B5CF5' },
  stats: { focus: 78, speed: 65, endurance: 82, memory: 71 },
  equippedItems: { hat: 'item-hat-2', top: null, bottom: null, shoes: null, hair: null, accessory: 'item-acc-1', background: null },
  featuredBadges: ['badge-first-blood', 'badge-speed-learner', 'badge-7-streak'],
  featuredItems: ['item-hat-2', 'item-acc-1'],
  weeklyXp: 6920, weeklyXpPrev: 5400,
  todayHours: 4.2, dailyGoal: 6, examDaysLeft: 89, examGoalHours: 600,
};

export const mockTasks: Task[] = [
  { id: 't1', name: 'Türev ve İntegral', subject: 'Matematik', subjectColor: '#7B5CF5', subjectEmoji: '📐', startTime: '09:00', endTime: '11:00', duration: 120, coins: 2, status: 'done', xp: 180 },
  { id: 't2', name: 'Elektrik Devreleri', subject: 'Fizik', subjectColor: '#22D3EE', subjectEmoji: '⚡', startTime: '11:30', endTime: '13:00', duration: 90, coins: 1.5, status: 'active', xp: 135, elapsed: 2340 },
  { id: 't3', name: 'Organik Kimya', subject: 'Kimya', subjectColor: '#F59E0B', subjectEmoji: '⚗️', startTime: '14:00', endTime: '15:30', duration: 90, coins: 1.5, status: 'pending', xp: 135 },
  { id: 't4', name: 'Hücre Bölünmesi', subject: 'Biyoloji', subjectColor: '#10B981', subjectEmoji: '🌿', startTime: '16:00', endTime: '17:00', duration: 60, coins: 1, status: 'pending', xp: 90 },
  { id: 't5', name: 'Şiir Analizi', subject: 'Edebiyat', subjectColor: '#EC4899', subjectEmoji: '📖', startTime: '17:30', endTime: '18:30', duration: 60, coins: 1, status: 'pending', xp: 90 },
];
...
export const mockLeaderboard: LeaderboardEntry[] = [
  { rank: 1, name: 'Ahmet K.', emoji: '🦁', level: 22, score: 9840, xp: 4800, maxXp: 6000, trend: 0, totalHours: 412 },
  { rank: 2, name: 'Zeynep A.', emoji: '🌸', level: 20, score: 8720, xp: 3900, maxXp: 5500, trend: 1, totalHours: 389 },
  { rank: 3, name: 'Burak T.', emoji: '⚡', level: 18, score: 7650, xp: 3200, maxXp: 5000, trend: -1, totalHours: 354 },
  { rank: 4, name: 'Elif M.', emoji: '🔥', level: 17, score: 7200, xp: 2800, maxXp: 4800, trend: 2, totalHours: 312 },
  { rank: 5, name: 'Shadow Fox', emoji: '🦊', level: 14, score: 6920, xp: 3420, maxXp: 5000, trend: 2, totalHours: 247, isMe: true },
  { rank: 6, name: 'CyberSage', emoji: '🧙', level: 15, score: 6540, xp: 2100, maxXp: 4500, trend: -1, totalHours: 298 },
  { rank: 7, name: 'MoonChild', emoji: '🌙', level: 13, score: 6120, xp: 1800, maxXp: 4000, trend: 0, totalHours: 234 },
  { rank: 8, name: 'TechNinja', emoji: '🥷', level: 12, score: 5890, xp: 1500, maxXp: 3800, trend: 1, totalHours: 221 },
];
...
export const mockMarketItems: MarketItem[] = [
  ...
  { id: 'item-feature-blue-tick', name: 'Mavi Tik ✓', category: 'ozellik', emoji: '✓', rarity: 'legendary', price: 500, owned: false, equipped: false },
];
...
export const mockVoiceRooms: VoiceRoom[] = [
  { id: 'vr1', name: 'Lo-fi Çalışma', emoji: '🎧', capacity: 20, occupied: 12 },
  { id: 'vr2', name: 'Sessiz Odak', emoji: '🔇', capacity: 15, occupied: 8 },
  { id: 'vr3', name: 'Kafe Ortamı', emoji: '☕', capacity: 10, occupied: 5 },
  { id: 'vr4', name: 'Sınav Öncesi', emoji: '📅', capacity: 10, occupied: 3 },
];
...
export const mockWeeklyCalendar: Record<string, WeeklyCalendarEntry[]> = {
  Mon: [
...
export const mockSubjectDistribution: SubjectDistribution[] = [
...
export const mockEvolutionTree: EvolutionTier[] = [
...
export const mockCommunityPosts: CommunityPost[] = [
  ...
  },
  {
    ...
    userId: 'u2', userName: 'NightWolf', userEmoji: '🐺', userColor: '#7B5CF5',
    content: 'Kim benimle matematik kafası yapıyor? 1 saat sprint!',
  ...
  {
    id: 'p2', type: 'post',
    userId: 'u4', userName: 'CyberSage', userEmoji: '🧠', userColor: '#22D3EE',
    content: 'Fizik sorusunu sonunda çözdüm! Elektrik devreleri artık kafamda net. 📚⚡',
    reactions: [{ emoji: '🎉', count: 15 }, { emoji: '❤️', count: 6 }],
    timestamp: '15 dk önce',
  },
  {
    id: 'p3', type: 'challenge',
    userId: 'u5', userName: 'StarGazer', userEmoji: '⭐', userColor: '#F59E0B',
    content: '30 dakika kimya çalışma odası açıyorum, gelin! 🧪',
  ...
  {
    id: 'p4', type: 'post',
    userId: 'u6', userName: 'IronMind', userEmoji: '🏆', userColor: '#10B981',
    content: 'Bu hafta 30 saat geçti, yeni rekor! Rank atlıyorum galiba 😤',
    reactions: [{ emoji: '🔥', count: 24 }, { emoji: '👑', count: 11 }],
    timestamp: '1 sa önce',
  },
  {
    id: 'p5', type: 'challenge',
    userId: 'u7', userName: 'MoonChild', userEmoji: '🌙', userColor: '#A78BFA',
    content: '2 saatlik biyoloji maratonu --- sadece kararlılar gelsin 🌿',
  ...
  {
    id: 'p6', type: 'post',
    userId: 'u8', userName: 'TechNinja', userEmoji: '🥷', userColor: '#EF4444',
    content: 'YKS ye 89 gün kaldı. Hâlâ burada mıyız? Evet. Devam. 🔥',
...
export const mockTaskHistory: TaskHistoryEntry[] = Array.from({ length: 30 }, (_, i) => {
  ...
  return { date: d.toISOString().split('T')[0], done, total };
});
